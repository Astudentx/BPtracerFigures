...
# 总和风险得分
# 1. 计算每个基因组的综合风险得分（总和）
gene_scaled$TotalRiskScore <- rowSums(gene_scaled[, 1:rader_edge])  # 或者你具体的列名范围
# 2. 按照总和打分进行五分位划分
quantiles <- quantile(gene_scaled$TotalRiskScore, probs = seq(0, 1, 0.2))
# 3. 使用 cut() 将总和映射到五个风险等级
...
group_means <- function(df) {
  colMeans(df[, 2:c(rader_edge+1)])  # 提取功能基因的列
}
...
# 设置PDF输出
pdf("03.figureUP/M3_Radarchart.pdf", width = 7, height = 4)
...
radarchart(radar_data,
 ...
 caxislabels = c("0", "", "50%", "", "100%"),  # 只显示关键刻度
  # caxislabels = c(0,1),
...
# 创建单独的图例文件
pdf("03.figureUP/M3_RadarchartLegend.pdf", width = 5, height = 5)
plot.new()
legend("center",
       legend = categories,
       bty = "n",
...
pdf("03.figureUP/M3_RadarchartLegend2.pdf", width = 17, height = 2)
plot.new()
legend("center",
       legend = categories,
       bty = "n",
       ...
       cex = 1.2,
       pt.cex = 2,
       ncol = length(categories),  # 设置列数等于类别数量（横向排列）
       x.intersp = 0.5)  # 调整水平间距
dev.off()
...
<!-- # 设置PDF输出 - 只包含总图 -->
<!-- pdf("03.figureUP/M3_Radarchart_Overview.pdf", width = 6, height = 6) -->
<!-- par(mar = c(2, 2, 3, 2)) -->
...
<!-- # 设置PDF输出 - 分图 -->
<!-- pdf("03.figureUP/M3_Radarchart_Individual.pdf", width = 8, height = 5) -->
...
temp$species_taxid <- as.numeric(temp$species_taxid) # 转为数值型
...
P_risk_cor <- ggplot(Risk_cor, aes(x = PathogenRiskScore, y = log(RadarArea+0.0001))) +
  ...
  geom_jitter(
    aes(color = Type),
    alpha = 0.3,           # 增加透明度，让重叠点可见
    size = 0.1,            # 稍微减小点的大小
    width = 0.2,           # 控制x方向的抖动范围
    height = 0.1,          # 控制y方向的抖动范围
    shape = 16             # 使用实心圆点
  ) +
  scale_x_continuous(
  ...
  geom_smooth(method = lm, show.legend = F, color = "black",size=1) +
  # stat_cor() +
  stat_cor( method = "spearman",show.legend = F)+
  theme_jgfbw() +
...
P_risk_cor
ggsave(filename = "03.figureUP/M3_RadarchartCor.pdf",height = 4,width = 3.8)
...
plot_sankey_visualization(prefix = "AllGene",Links = sankey1$sankey_linklist,sankey1$sankey_nodelist,color = color_scheme_bp("color_zyz6")[5:8],path = "03.figureUP/M3_Sankey/",height = 350,width = 500)



```

# 附图-数据库百分比单饼图（Type and Phylum）

```{r}


# dataframe:长型表
# value：字符串,df中需要计算比例的value一列
# label：字符串,标签列，或者是ID列
...
for (i in c(1:5)) {
  ...
  DB_type$ID <- str_to_title(DB_type$ID) #首字母大写
  # top前10的Type
  if (nrow(DB_type)>5) {
    Top <- 8
  ...
  barnname=paste0("03.figureUP/S3_Pie/Pie_state_Type_", DB_name)
...
for (i in c(1:5)) {
  ...
  xlab('')+ylab('')+
  guides(fill=guide_legend( ncol=1))
  barnname=paste0("03.figureUP/S3_Pie/Pie_state_PA_", DB_name)
...
for (i in c(1:5)) {
  ...
  DB_Phylum$ID <- str_to_title(DB_Phylum$ID) #首字母大写
  # top前10的Phylum
  if (nrow(DB_Phylum)>5) {
    Top <- 5
  ...
  xlab('')+ylab('')+
  guides(fill=guide_legend( ncol=1))
  barnname=paste0("03.figureUP/S3_Pie/Pie_state_Phylum_", DB_name)
...
Final
ggsave(Final,file="03.figureUP/S3_Pie/Pie_final.pdf",width = 12,height = 10) 
...
ggsave(filename = "03.figureUP/M3_biopollutomeStatCol.pdf",height = 5.2,width = 10)
...
gene_kind_percentage <- temp1 %>%
  ...
  # 保留每个 GeneKind 只显示一次结果
  distinct(GeneKind, .keep_all = TRUE)

# 查看结果
cat("病原菌物种占5种功能基因中的比例：", "%\n")
...
p1 <- ggplot(data = tTest_gene_final[,], aes(x = Type2, y = Freq, fill = Type2)) +
  ...
  scale_y_continuous(trans = "sqrt", labels = scales::scientific) +  # 🔹平方根或改为 log10
...
# ggsave(filename = "03.figureUP/S_biopollutome_compare.pdf",height = 3.6,width = 12)
...
ggsave(filename = "03.figureUP/S2_biopollutome_compare.pdf",height = 8,width = 15)
...
```{r}
attach(HI_tax)
cat("Refseq下载基因中病原菌基因组占比：", "%\n")
uniq_c(HI_tax$Type)

cat("Refseq下载基因中病原菌物种占比：", "%\n")
...
# 计算每类基因在 Pathogen 与 NonPathogen 中的平均 Freq
